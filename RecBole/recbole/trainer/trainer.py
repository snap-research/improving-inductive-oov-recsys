...
class Trainer(AbstractTrainer):
    ...
    def evaluate(
        ...
        if isinstance(eval_data, FullSortEvalDataLoader):
            ...
            # enable sampled evaluation if the flag is set
            if self.config['sample_eval']:
                eval_data.set_sampling(True)
        ...
        # re-disable eval in case we need it off later
        if isinstance(eval_data, NegSampleEvalDataloader):
            eval_data.set_sampling(False)
        return result
...
class InductiveTrainer(Trainer):
    """InductiveTrainer is the trainer class for inductive recommendation models.
    It features an additional OOV training step that can optionally freeze the embeddings for IV IDs.
    """
    def __init__(self, config, model: Union[InductiveContextRecommender, InductiveGeneralRecommender]):
        """Initialize the InductiveTrainer.

        Args:
            config (dict): The configuration dictionary.
            model (Union[InductiveContextRecommender, InductiveGeneralRecommender]): The inductive recommender model.
        """
        super().__init__(config, model)
        self.inductive_mapper: Union[AbstractInductiveMapper, None] = None
        self.inductive_embedder: Union[AbstractInductiveEmbedder, None] = None
        self.oov_train_ratio = config['oov_train_ratio']
        self.oov_feature_mask_rate = config['oov_feature_mask_rate']
        self.valid_sample_ratio = config['eval_valid_sample_ratio']

    def set_inductive_mapper(self, inductive_mapper: AbstractInductiveMapper):
        self.inductive_mapper = inductive_mapper

    def set_inductive_embedder(self, inductive_embedder: AbstractInductiveEmbedder):
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if self.config['train_oov'] and self.config['oov_only_epoch']:
                if self.config['oov_freeze_embedding'] and self.config['oov_freeze_skip_optim']:
                    opt_clone = copy.deepcopy(self.optimizer)
                self.model.set_oov_train()
                oov_loss = self._train_oov(
                    train_data, epoch_idx, show_progress=show_progress
                )
                self.model.set_oov_eval()
                if self.config['oov_freeze_embedding'] and self.config['oov_freeze_skip_optim']:
                    self.optimizer = opt_clone
            ...
            to_log = {"epoch": epoch_idx, "train_loss": train_loss, "train_step": epoch_idx}
            if self.config['train_oov'] and self.config['oov_only_epoch']:
                to_log['oov_loss'] = oov_loss
                if oov_loss is not None:
                    to_log['total_loss'] = train_loss + oov_loss
                else:
                    to_log['total_loss'] = train_loss
            self.wandblogger.log_metrics(to_log, head="train")
    ...
    def _augment_with_oov(self, interaction):
        int_copy = copy.deepcopy(interaction)

        # select approximately r * n_int samples
        selection_mask = torch.rand(len(interaction)) < self.oov_train_ratio
        perm_mask = torch.randperm(len(interaction) + selection_mask.sum())
        for col in int_copy.columns:
            int_copy[col] = (int_copy[col][selection_mask])
        self._transform_interaction_oov(int_copy)

        for col in interaction.columns:
            interaction[col] = torch.concat((interaction[col], int_copy[col]))
            interaction[col] = interaction[col][perm_mask]
        return interaction
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        """
        self.model.train()
        if self.config['train_oov'] and not self.config['oov_only_epoch']:
            self.model.set_oov_train(no_freeze=True)
        loss_func = loss_func or self.model.calculate_loss
        total_loss = None
        iter_data = (
            tqdm(
        ...
        for batch_idx, interaction in enumerate(iter_data):
            if self.config['train_oov'] and not self.config['oov_only_epoch']:
                interaction = self._augment_with_oov(interaction)
            ...
            if self.config['oov_debug_skip_train']:
                break

        if self.config['train_oov'] and not self.config['oov_only_epoch']:
            self.model.set_oov_eval(no_freeze=True)
        return total_loss

    def _transform_interaction_oov(self, interaction):
        option = random.choice([0, 1, 2])
        if option in (0, 2):
            interaction[self.model.ITEM_ID] += self.config['oov_prime_pad']
        if option in (1, 2):
            interaction[self.model.USER_ID] += self.config['oov_prime_pad']

        if self.oov_feature_mask_rate > 0:
            for col in interaction.columns:
                mask = torch.rand_like(interaction[col], dtype=torch.float) < self.oov_feature_mask_rate
                interaction[col][mask] = 0
        return interaction

    def _train_oov(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        r"""Do an epoch of OOV training
        ...
        iter_data = (
            tqdm(
                ...
                total=len(train_data),
                ncols=100,
                desc=set_color(f'Train OOV {epoch_idx:>5}', 'pink'),
            )
            if show_progress
            else train_data
        )
        ...
        scaler = amp.GradScaler(enabled=self.enable_scaler)
        for batch_idx, interaction in enumerate(iter_data):
            # TODO(wshiao): this is very inefficient, we should sample at an earlier stage
            if random.random() > self.oov_train_ratio:
                continue
            interaction = interaction.to(self.device)
            interaction = self._transform_interaction_oov(interaction)
    ...
    def _valid_epoch(self, valid_data, show_progress=False):
        ...
        """
        valid_result = self.evaluate(
            valid_data, load_best_model=False, show_progress=show_progress, sample_eval_ratio=self.valid_sample_ratio
        )
        valid_score = calculate_valid_score(valid_result, self.valid_metric)
        return valid_score, valid_result
    ...
    @torch.no_grad()
    def evaluate(
        self, eval_data, load_best_model=True, model_file=None, show_progress=False, sample_eval_ratio = None
    ):
        r"""Evaluate the model based on the eval data.
        ...
        if sample_eval_ratio is not None and (0 < sample_eval_ratio < 1):
            print('Sampling evaluation with invalid ratio: ', sample_eval_ratio, ' - turning off')
            sample_eval_ratio = None

        num_sample = 0
        for batch_idx, batched_data in enumerate(iter_data):
            # Note: extremely inefficient, but works for now
            if batch_idx >= 1 and sample_eval_ratio is not None:
                if random.random() > sample_eval_ratio:
                    continue
            ...
            if self.config['oov_debug_skip_eval']:
                break
        self.eval_collector.model_collect(self.model)
        struct = self.eval_collector.get_data_struct()
        result = self.evaluator.evaluate(struct)
